...
class SessionResultView(APIView):
    ...
    Returns restaurants sorted by like count.
    """
    def get(self, request, code):
        session = get_object_or_404(Session, code=code)
...
class SessionFetchRestaurantsView(APIView):
    ...
    def post(self, request, code):
        ...
        try:
            ...
            logger.info(f'Linked {len(restaurants)} restaurants to session {session.code}')
...
class SessionCompleteView(APIView):
    ...
    Mark a session as completed after voting is done.
    ...
    def post(self, request, code):
        ...
        if session.creator_name != creator_name:
            return Response(
                {'error': 'Only the session creator can complete the session'},
        ...
        return Response({
            'message': f'Session {code} has been marked as completed',
            'session_code': session.code,
            'status': session.status,
            'total_participants': session.participants.count(),
